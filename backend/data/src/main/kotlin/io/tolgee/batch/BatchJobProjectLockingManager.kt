...
class BatchJobProjectLockingManager(
  ...
  private fun getRedissonProjectLocks(): ConcurrentMap<Long, Set<Long>> {
    return try {
      getRedissonProjectLocksV2()
    } catch (e: ClassCastException) {
      logger.warn("Detected old Redis format for key $REDIS_PROJECT_BATCH_JOB_LOCKS_KEY, performing one-time migration to new format")
      migrateToV2()
      getRedissonProjectLocksV2()
    }
  }
  ...
  private fun migrateToV2() {
    ...
    val newMap = getRedissonProjectLocksV2()
    localCopy.forEach { (projectId, jobSet) ->
      newMap[projectId] = jobSet
    }

    logger.info("Successfully migrated ${localCopy.size} project locks from old format to new format")
  }

  // For migration purposes only. Should only ever be run once
  private fun getRedissonProjectLocksV1(): RMap<Long, Long> {
    return redissonClient.getMap(REDIS_PROJECT_BATCH_JOB_LOCKS_KEY)
  }

  private fun getRedissonProjectLocksV2(): RMap<Long, Set<Long>> {
    return redissonClient.getMap(REDIS_PROJECT_BATCH_JOB_LOCKS_KEY)
  }
